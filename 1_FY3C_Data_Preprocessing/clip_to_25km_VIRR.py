...
if __name__ == '__main__':
    ...
    # 目录设置
    dir = data_dir+ r'\VIRR_LST'  # LST数据目录
    # 获取所有LST文件
    file_list = [os.path.join(dir, f) for f in os.listdir(dir) if f.endswith('_LST.tif')]
    print(f"找到 {len(file_list)} 个VIRR LST文件待处理")

    out_dir = os.path.join(data_dir, 'Sub_to_25km\VIRR_LST_25km')
    ...
    for file in file_list:
        ...
        # 构建输出文件名
        outfile = os.path.join(out_dir, f'FY3C_VIRR_{date}_25km_LST.tif')
        # 进行重投影
